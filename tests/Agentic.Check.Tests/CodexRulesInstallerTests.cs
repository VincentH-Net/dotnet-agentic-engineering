...
public sealed class CodexRulesInstallerTests
{
    ...
    public void PlanInstallsAtTheTargetWhenNoFileExists()
    {
        ...
        Assert.Equal(Path.Combine(tempDirectory.Path, ".codex", "rules", "dotnet-agentic-engineering.rules"), plan.File);
        Assert.Equal(Path.Combine(".codex", "rules", "dotnet-agentic-engineering.rules"), plan.Display);
    ...
    {
        using TempDirectory tempDirectory = new();
        tempDirectory.Write(".codex/rules/dotnet-agentic-engineering.rules", "prefix_rule(pattern = [\"dotnet\"])\n");
        var plan = CodexRulesInstaller.Plan(tempDirectory.Path);
    ...
    public void OutdatedFileAtTheGitRootIsUpdatedThereNotCopiedBelow()
    {
        using TempDirectory tempDirectory = new();
        _ = tempDirectory.CreateDirectory(".git");
        tempDirectory.Write(".codex/rules/dotnet-agentic-engineering.rules", "stale\n");
        string subfolder = tempDirectory.CreateDirectory("backend");
    ...
    public void FileBelowTheTargetOrAboveTheGitRootDoesNotCount()
    {
        ...
        tempDirectory.Write("repo/backend/.codex/rules/dotnet-agentic-engineering.rules", CodexRulesInstaller.Content);

        var plan = CodexRulesInstaller.Plan(repo);

        Assert.False(plan.FileExists);
        Assert.Equal(Path.Combine(repo, ".codex", "rules", "dotnet-agentic-engineering.rules"), plan.File);
    }
    ...
    public void WithoutAGitRootOnlyTheTargetCounts()
    {
        ...
        Assert.False(plan.FileExists);
        Assert.Equal(Path.Combine(subfolder, ".codex", "rules", "dotnet-agentic-engineering.rules"), plan.File);
    }
