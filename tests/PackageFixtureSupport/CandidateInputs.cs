...
sealed record CandidateBuild(string Branch, string Commit, string Configuration, DateTimeOffset PackedAtUtc, PackageArtifact Check, PackageArtifact Companion);
...
static partial class CandidateInputs
{
    ...
    internal static async Task<CandidateBuild> LoadAsync()
    {
        ...
        FixtureFiles.Require(check.Sha256 == build.Check.Sha256 && companion.Sha256 == build.Companion.Sha256, "Candidate hashes differ from build provenance.");
        FixtureFiles.Require(check.RepositoryCommit == build.Commit && companion.RepositoryCommit == build.Commit, "Package repository commits differ from build provenance.");
        ...
        return build with { Check = check, Companion = companion };
    }
    ...
    internal static async Task PackAsync(string outputDirectory, string configuration)
    {
        ...
        RealProcess process = new();
        foreach (string project in new[] { "src/Agentic.Check/Agentic.Check.csproj", "src/Agentic/Agentic.csproj" })
        ...
        FixtureFiles.WriteJson(Path.Combine(output, "candidate-build.json"), new CandidateBuild(branch, sha, configuration, DateTimeOffset.UtcNow, check, companion));
    }
