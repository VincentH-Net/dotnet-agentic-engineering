    ...
    public async Task PackagedCompanionRunsInRecordedTerminalFromLocalManifestAndChildFolder(bool shorthand)
    {
        ...
        await using (terminal.ConfigureAwait(true))
        {
            ...
            try
            {
                ...
                string defaultShow = await CommandAsync("clear; " + command + " prompt-log --limit 1 -m 2.3", 0).ConfigureAwait(true);
                Assert.Contains("older entries omitted", defaultShow, StringComparison.Ordinal);
                Assert.Contains("old last line", defaultShow, StringComparison.Ordinal);
                Assert.DoesNotContain("[red]literal[/]", defaultShow, StringComparison.Ordinal);
                string allLogs = await CommandAsync("clear; " + command + " prompt-log --all -m 2.3", 0).ConfigureAwait(true);
                Assert.Contains("legacy prompt log (JSON)", allLogs, StringComparison.Ordinal);
                Assert.Contains("[red]literal[/]", allLogs, StringComparison.Ordinal);
                Assert.DoesNotContain("older entries omitted", allLogs, StringComparison.Ordinal);
                string bare = await CommandAsync("clear; " + command + " prompt-log", 0).ConfigureAwait(true);
                Assert.Contains("[red]literal[/]", bare, StringComparison.Ordinal);
                Assert.Contains("old last line", bare, StringComparison.Ordinal);
                string promptHelp = await CommandAsync("clear; " + command + " prompt-log --help", 0).ConfigureAwait(true);
                Assert.Contains("--limit", promptHelp, StringComparison.Ordinal);
                Assert.Contains("wrap", promptHelp, StringComparison.Ordinal);
                Assert.DoesNotContain("old last line", promptHelp, StringComparison.Ordinal);
                _ = await CommandAsync(command + " prompt-log --all --limit 1", 2).ConfigureAwait(true);
