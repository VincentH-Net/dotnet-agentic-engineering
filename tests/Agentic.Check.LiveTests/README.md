...
4. A candidate that must wait for something outside its repo, such as a skill for a control that has not shipped, is recorded in `SkillDeferrals.cs` with the repo and path whose presence in that repo's latest release ends the wait. The report lists it under "Deferred candidates" and the test passes; the baseline may advance. Once the trigger path is released, it is a new candidate again until included or excluded.
